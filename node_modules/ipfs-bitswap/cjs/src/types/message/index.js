...
class BitswapMessage {
  ...
  serializeToBitswap110() {
    ...
    for (const [cidStr, data] of this.blocks.entries()) {
      ...
      const codec = Uint8Array.from([cid$1.code]);
      const multihash = cid$1.multihash.bytes.subarray(0, 2);
      const prefix = concat.concat([
        [cid$1.version],
        codec,
        multihash
...
BitswapMessage.deserialize = async (raw, hashers = {}) => {
  ...
  if (decoded.payload.length > 0) {
    await Promise.all(decoded.payload.map(async p => {
      ...
      const hasher = hashAlg === sha2.sha256.code ? sha2.sha256 : hashers[hashAlg];
      if (!hasher) {
